...
const AnnounceWrite = () => {
    ...
    const handleAnnounce = async (e) => {
        e.preventDefault();
        await axios.post('/api/announce/write', {
    ...
    return (
        <>
            <div className="absolute left-[487px] top-[146px] w-[466px] flex flex-col items-end justify-start gap-[31px]">
                <div className="self-stretch text-[32px] leading-[50px] font-['Roboto'] font-bold text-[#000] text-center">공지사항</div>

                <form onSubmit={handleAnnounce} id='form' autoComplete='off' className="self-stretch flex flex-col items-center justify-center gap-[30px]">
                    <div className="self-stretch flex flex-col items-start justify-start gap-[5px]">
                        ...
                        <div className="self-stretch h-[44px] shrink-0 flex flex-row items-center justify-start p-[10px] border-[1px] border-solid border-[#00000080] rounded-[10px]">
                            <input onInput={validateTitle} id='title' placeholder='제목을 입력해주세요.' type='text' name='aTitle' onChange={(e) => setValues({ ...values, aTitle: e.target.value })} className="flex-1 text-[16px] leading-[25px] font-['Roboto'] text-[#000] outline-none"></input>
                        </div>
                    ...
                    <div className="self-stretch flex flex-col items-start justify-start gap-[5px]">
                        ...
                        <div className="self-stretch h-[300px] shrink-0 flex flex-row items-start justify-start p-[10px] border-[1px] border-solid border-[#00000080] rounded-[10px]">
                            <textarea onInput={validateContent} id='content' placeholder='내용을 입력해주세요.' type='text' name='aContent' onChange={(e) => setValues({ ...values, aContent: e.target.value })} className="flex-1 self-stretch text-[16px] leading-[25px] font-['Roboto'] text-[#000] outline-none resize-none"></textarea>
                        </div>
                        <div id='contentError' className="self-stretch text-[16px] leading-[25px] font-['Roboto'] text-[#f00]"></div>
                    </div>
                </form>

                <button disabled id='announce' type='submit' form='form' className="flex flex-row items-center justify-center py-[12px] px-[24px] bg-[#0090f9] rounded-[10px]">
